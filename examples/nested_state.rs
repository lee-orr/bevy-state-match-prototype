...
fn main() {
    App::new()
        ...
        .add_systems(
            Entering,
            setup_paused.run_in(AppState::InGame(GameState::Paused)),
        )
        ...
        .add_systems(Entering, setup_game.run_in(state_matches!(AppState, InGame(_))))
        // You can also pass in a closure to the macro, like so
        .add_systems(
            Entering,
            setup_in_game_ui.run_in(state_matches!(AppState, |state: &AppState| state
        ...
        .add_systems(Update, menu.run_in(AppState::Menu))
        .add_systems(
            Update,
            change_color.run_in(state_matches!(AppState, InGame(_))),
        )
        // Lastly, for closures that implement [`StateMatcher`], we can actually pass them in directly as well
        .add_systems(
            Update,
            invert_movement.run_in(|state: &AppState| matches!(state, AppState::InGame(GameState::Running { .. }))),
        )
        .add_systems(
            Update,
            movement.run_in(AppState::InGame(GameState::Running {
                inverted: false,
            })),
        )
        .add_systems(
            Update,
            inverted_movement.run_in(AppState::InGame(GameState::Running {
                inverted: true,
            })),
        )
        .run();
