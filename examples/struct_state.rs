...
fn main() {
    App::new()
        ...
        .add_systems(
            Entering,
            setup_menu.run_in(state_matches!(AppState, AppState { in_menu: true, .. })),
        )
        .add_systems(
            Entering,
            setup_paused.run_in(state_matches!(
                AppState,
                AppState {
                    is_paused: true,
                    ..
        ...
        .add_systems(
            Entering,
            setup_game.run_in(state_matches!(AppState, |state: &AppState| {
        ...
        .add_systems(
            Entering,
            setup_in_game_ui.run_in(|state: &AppState| state.in_game.is_some() && !state.is_paused),
        )
        ...
        .add_systems(
            Update,
            menu.run_in(state_matches!(AppState, AppState { in_menu: true, .. })),
        )
        .add_systems(
            Update,
            change_color.run_in(state_matches!(AppState, |state: &AppState| state
                .in_game
                .is_some())),
        )
        .add_systems(
            Update,
            invert_movement.run_in(state_matches!(
                AppState,
                AppState {
                    in_game: Some(_),
                    is_paused: false,
        ...
        .add_systems(
            Update,
            inverted_movement.run_in(AppState {
                in_game: Some(true),
                is_paused: false,
                in_menu: false,
            }),
