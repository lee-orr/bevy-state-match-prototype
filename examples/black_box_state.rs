...
fn main() {
    App::new()
        ...
        .add_systems(Entering, setup_menu.run_in(in_menu))
        // You can also use impl's to mimic enums, such as here to see if we're paused
        .add_systems(Entering, setup_paused.run_in(GameState::paused))
        // And here to see if we are in any game state
        .add_systems(Entering, setup_game.run_in(GameState::any))
        // or that we are specifically not paused
        .add_systems(Entering, setup_in_game_ui.run_in(GameState::running))
        // you can also use `Fn` values rather than points, like we do here
        .add_systems(Update, change_color.run_in(in_game()))
        // Or pass in functions that test the full transition rather than just a single state
        .add_systems(Exiting, cleanup_ui.run_in(ui))
        .add_systems(Update, menu.run_in(in_menu))
        .add_systems(Update, invert_movement.run_in(GameState::running))
        // You can also use generics to specialize things
        .add_systems(Update, movement.run_in(in_movement::<Standard>))
        .add_systems(Update, inverted_movement.run_in(in_movement::<Inverted>))
        .run();
}
use state::*;
mod state {
...
fn menu(
    mut next_state: ResMut<NextMatchableState<AppState>>,
...
fn toggle_pause(input: Res<Input<KeyCode>>, mut next_state: ResMut<NextMatchableState<AppState>>) {
